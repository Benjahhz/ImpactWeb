...
const observer = new IntersectionObserver((entries) => {
  entries.forEach((entry) => {
    ...
    if (entry.isIntersecting) {
        ...
        $(entry.target).find('.desarrollo__item__image').eq(0).addClass('animate__animated animate__fadeInLeft animate__normal');
        $(entry.target).find('.desarrollo__item__image').eq(1).addClass('animate__animated animate__fadeInRight animate__normal');
        setTimeout(() => {
